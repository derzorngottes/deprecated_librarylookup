...
module.exports = {
  ...
  getBookFromId = function(queryId) {
    return knex('books').where({ id: queryId });
  },
  ...
  editBook = function(metadata) {
    return knex('books').update({ title: metadata.title, genre: metadata.genre, coverUrl: metadata.coverUrl, description: metadata.description, authors: metadata.authors });
  }
